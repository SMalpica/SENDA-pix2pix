...
if __name__ == '__main__':
    ...
    datafolder = 'water/DEM/val'
    outfolder = 'water/DEM_3_normalized/val'
    ...
    for filename in os.listdir(datafolder):
        ...
        tpi = TPI(elevGrid, 15)
        # normalizamos usando la standard deviation (3 sigma, de hecho) para que la mayoría de valores se queden
        # entre -1 y 1. Luego además paso el rango a 0..1
        ...
        if plot == True:
            ...
            print('Percentile 99.9%:', np.percentile(tpi, 99.9))
            plt.figure(figsize=(10, 10))
            plt.imshow(tpi, vmin=0, vmax=1, cmap='viridis')
            plt.colorbar()
            plt.show()
        ...
        # elevGridBreached = rd.FillDepressions(elevGridBreached, epsilon=True)
        elevGridBreached = rd.FillDepressions(rd.rdarray(elevGrid, no_data=-9999), epsilon=True)

        # compute flow using Dinf algorithm
        # saRD = rd.FlowAccumulation(elevGridBreached, method='Dinf')
        ...
        if plot == True:
            fig = plt.figure(figsize=(20, 20))
            plt.imshow(wi[pimin:pimax, pjmin:pjmax], cmap='coolwarm',
                       vmin=np.percentile(wi, 0.1), vmax=np.percentile(wi, 99.5))
            plt.show()
